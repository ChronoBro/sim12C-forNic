...
int main()
{
  ...
  float thickness = 9.472;  //mg/cm2
  float const targetSize = 0.4;//0.4;//0.5; // spotsize at target in cm
  float const spotSizeAtHira = 0.4;//0.4;//0.4;//0.5; //spotsize at detector in cm
  float CsiRes = .085;
  //float CsiRes = .085/2.;
  ...
  CPlf plf(thickness);
  //CFrag frag1(1.,3.,LossH,CsiRes,thickness,H3thres);
  ...
  CFrag frag1(2.,4.,LossHe,CsiRes,thickness,He4thres);
  CFrag frag2(2.,4.,LossHe,CsiRes,thickness,He4thres);
 ...
 hist_E_alpha_R.GetXaxis()->SetTitle("TKE\\, of \\, Alpha \\,\\, (reconstructed)\\,\\ [MeV]");//stuff i added
  ...
  TH1I hist_Ex_R("Ex_R","Ex",700,-3,10.);
  ...
  for (int i=0;i<Nevents;i++)
      ...
      if (i==3)
	{
      ...
      float ET = 0.0918;//-<8Be//2.185;//0.7117;//<-for 6Li // 2.185; <-for 7Li
      const float c = 30.;
      ...
      float Etot = (EPA2+931.478)*7.+mass_target*931.478;
      hist_EPA.Fill(EPA2);
	...
	sqrt(pow(p,2)+pow((mass_target)*931.478,2));
      ...
      //after interaction
      EtotRCOM -= ET;//1.6052;//1.5866;//2.468;//1.5866;
      ...
      for(;;)
	{
	  float y =  sqrt(pow(pafter,2)+pow(8.*931.478,2)) + sqrt(pow(pafter,2)+pow((mass_target-1.)*931.478,2)); //+pow(7.*931.478,2)) + 
	  
	  float dy = pafter/sqrt(pow(pafter,2)+pow(8.*931.478,2)) +pafter/sqrt(pow(pafter,2)+pow((mass_target-1.)*931.478,2)); //+pow(7.*931.478,2)) + 
     ...
     */
     ...
     //total energy in lab
     float Epp = sqrt(pow(pp,2)+pow(8.*931.478,2));//+pow(6.*931.478,2));//+pow(7.*931.478,2)); //<-change this line back to 7Li to have velocities similar 7Li (reproduce original plots)
	...
	frag2.recon->energy;//1.4743; 
      ...
      hist_Ex_R.Fill(ErelR-ET);//1.4743);
