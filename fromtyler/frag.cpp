	     ...
	     float thickness):fragment(Z0,mass0)
{
  ...
  Ring = new ring(290.,7.5,62.5,128,128,0); //S4
  Inactive = new ring(290.,6.5,7.5,1,1,0);//S4 inactive area
  // RingCsI = new ring(365.,7.5,62.5,128,128); //S4 back of CsI
  RingCsI = new ring(307.,7.5,62.5,128,128,1); //p range;
  //RingCsI1 = new ring(332.,7.5,62.5,128,128);
  //RingCsI = new ring(315.,7.5,62.5,128,128); // 0 thickness

  Plane = new plane_det(445.,20.,20.,0.5);
...
//**********************************************************
   ...
   * logical function determines if a particle is detected
   */
int CFrag::hit3(float xtarget,float ytarget, float straggle)
{ 

  rho = Ring->dist/cos(real->theta);
  rho_CsI = RingCsI->dist/cos(real->theta);
...
int CFrag::hit4(float xtarget,float ytarget)
{
  is_hit = Plane->hit(real->theta,real->phi,xtarget,ytarget) ;
...
int CFrag::hit5(float xtarget, float ytarget)
{
  rho = Inactive->dist/cos(real->theta);
  x = rho*sin(real->theta)*cos(real->phi) + xtarget;
  y = rho*sin(real->theta)*sin(real->phi) + ytarget;
  theta_prime = atan2(sqrt(pow(x,2.) + pow(y,2.)),Ring->dist);
  phi_prime = atan2(y,x)+acos(-1);
