{
  ...
  TH2S frame3("frame3","",10,0,360,10,-200,1400);
  ...
  TLatex tt;
  tt.DrawLatex(40,1300,"^{7}Be#rightarrow ^{3}He+#alpha,  #theta^{*}> 4.5^{#circ}");
  tt.DrawLatex(250,1300,"j=7/2^{-}, l=3");


  TH1I* deltaPhi = (TH1I*) file.Get("deltaPhi_7Be_2_el_hiAngle");
  deltaPhi->SetMarkerStyle(20);
  //deltaPhi->Draw("same P");
  float Nexp2 = deltaPhi->Integral();

  TH1I* deltaPhi_sim = (TH1I*) fileS.Get("deltaPhi_hiAngle_R");
  ...
  TH1I* deltaPhi_sim_P = (TH1I*) fileS.Get("deltaPhi_hiAngle_P");
  ...
  for (int i=1;i<=90;i++)

    {
      ...
      deltaPhi_corr->SetBinError(i,y/sqrt(yexp));
