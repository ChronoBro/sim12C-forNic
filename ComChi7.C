...
gROOT->ForceStyle();
 gStyle->SetErrorX(0.);
 TCanvas canvas("xsec3_7Be","",500,800);
 ...
 double overlap1 = .04;
  ...
  cout << SLE.Y[0]/SLE.Y[0] << " " << SLE.Y[1]/SLE.Y[0] << " " << SLE.Y[2]/SLE.Y[0]<< " " << SLE.Y[3]/SLE.Y[0] << endl;
  ...
  for (int i=1;i<=90;i++)

    {
      ...
      fit->SetBinContent(i,y1*SLEE.Y[0]+y2*SLEE.Y[1]+y3*SLEE.Y[2]);


      fit1->SetBinContent(i,y1*SLEE.Y[0]);
      fit2->SetBinContent(i,y2*SLEE.Y[1]);

      fit3->SetBinContent(i,y3*SLEE.Y[2]);
